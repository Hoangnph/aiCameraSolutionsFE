#!/usr/bin/env python3
"""
🔍 Test with Real Face Image (Fixed)
Test với ảnh face thực tế - đã fix database constraints
"""
...
def create_unique_email():
    """Create unique email to avoid database constraints"""
...
def test_upload_with_real_face(image_path):
    ...
    try:
        ...
        with open(image_path, 'rb') as f:
            ...
            data = {
                'name': f'RealFaceTestUser_{uuid.uuid4().hex[:8]}',
...
def test_end_to_end_with_real_face():
    ...
    try:
        ...
        # Step 2: Upload face with unique data
...
def test_multiple_faces():
    ...
    for i in range(total_tests):
        ...
        if image_path:
            ...
            try:
                ...
                if response.status_code == 200:
                    print(f"✅ Test {i+1}: Upload successful")
                    success_count += 1
                else:
                    print(f"❌ Test {i+1}: Upload failed")
    ...
    print(f"\n📊 Multiple Faces Test: {success_count}/{total_tests} successful")
    return success_count == total_tests

def main():
    """Run tests with real face images"""
    print("🚀 Starting Real Face Tests (Fixed)")
    print("=" * 60)
    ...
    if image_path:
        ...
        multiple_success = test_multiple_faces()
    else:
        upload_success = recognition_success = workflow_success = multiple_success = False
    ...
    print(f"✅ Multiple Faces: {'PASS' if multiple_success else 'FAIL'}")
    ...
    return upload_success and recognition_success and workflow_success and multiple_success
